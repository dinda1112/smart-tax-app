    ...
    <div className="flex min-h-screen items-center justify-center bg-[var(--background)] px-4 py-12">
      <div className="w-full max-w-md space-y-8">
        <div className="text-center">
          <h1 className="text-3xl font-black tracking-tight text-[var(--text-primary)]">{t(language, "auth.login.title")}</h1>
          <p className="mt-2 text-sm text-[var(--text-secondary)]">{t(language, "auth.login.subtitle")}</p>
        </div>
          ...
          <div className="rounded-[18px] border border-[var(--border)] bg-[var(--surface)] p-6 transition-colors theme-shadow">
            <div className="space-y-4">
              ...
              <Button type="submit" className="w-full" disabled={loading}>
                {loading ? t(language, "auth.login.signingIn") : t(language, "auth.login.signIn")}
              </Button>
            </div>
          </div>
        </form>
